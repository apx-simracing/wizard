    ...
    def handle(self, *args, **options):
        ...
        for entry in entries:
            ...
            if not exists(component_path):
                self.stdout.write(
                    self.style.SUCCESS(
                        "Creating component path for {}".format(component_name)
                    )
                )
            ...
            for discovered_file in files:
                ...
                for needle in FILE_NAME_SUFFIXES:
                    ...
                    if discovered_file == file_suffix:
                        ...
                        entry_file = (
                            EntryFile.objects.filter(file__endswith=discovered_file)
                            .exclude(file__endswith=component_file_path_new)
                            .first()
                        )
                        ...
                        if entry_file is None:
                            ...
                            if not exists(join(MEDIA_ROOT, component_file_path_new)):
                                ...
                                entry_file = EntryFile.objects.filter(
                                    file__endswith=discovered_file
                                ).first()
                                ...
                                if entry_file is None:
                                    self.stdout.write(
                                        self.style.SUCCESS(
                                            "Adding new entry for existing file {}".format(
                                                discovered_file
                                            )
                                        )
                                    )
                                    ...
                                    self.stdout.write(
                                        self.style.SUCCESS(
                                            "File exists, and entry file exists {}".format(
                                                discovered_file
                                            )
                                        )
                                    )
                            ...
                            self.stdout.write(
                                self.style.SUCCESS(
                                    "Updating {} to {}".format(
                                        entry_file.pk, component_file_path_new
                                    )
                                )
                            )
